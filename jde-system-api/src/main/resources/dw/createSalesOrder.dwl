...
---
{
	ns0#createSalesOrder: {
		orderHeader: {
			...
			customerInfo: {
				tccCustomerId: payload.OrderHeader.customerInfo.shellCustomerId when payload.OrderHeader.customerInfo.shellCustomerId? and payload.OrderHeader.miscInfo.isInStorePickup == null and ('CONSUMER' == payload.OrderHeader.customerInfo.tccCustomerType)  otherwise payload.OrderHeader.customerInfo.tccCustomerId,
		...
		commerceItems: {
			(payload.commerceItems.commerceItem map ((commerceItem , indexOfCommerceItem) -> {
				commerceItem: {
					commerceItemInfo: {
						skuId: "ORDEREDSKU" when (commerceItem.skuIdType == 'original' and ('ORDERED' == commerceItem.itemStatus or 'CANCELLED' == commerceItem.itemStatus) and not (['ORDEREDDPAYVOL','ORDEREDSECDEP','ORDEREDDPAYMAN'] contains commerceItem.skuId)) and payload.OrderHeader.miscInfo.isInStorePickup == null otherwise commerceItem.skuId ,
						(originalSKUId: commerceItem.skuId) when payload.OrderHeader.miscInfo.isInStorePickup == null and commerceItem.skuIdType == 'original' and ('ORDERED' == commerceItem.itemStatus or 'CANCELLED' == commerceItem.itemStatus) and not (['ORDEREDDPAYVOL','ORDEREDSECDEP','ORDEREDDPAYMAN'] contains commerceItem.skuId),
					...
					itemPriceInfo: {
						itemPrice:{
							...
							itemPrice: commerceItem.amount when (payload.OrderHeader.miscInfo.isInStorePickup == null and commerceItem.skuIdType == 'original'  and ('ORDERED' == commerceItem.itemStatus or 'CANCELLED' == commerceItem.itemStatus)) otherwise (commerceItem.listPrice when commerceItem.listPrice? otherwise commerceItem.commissionReceivableAmount),
